...
class IntakeAppsScriptStaticTests(unittest.TestCase):
    def test_multi_campaign_apps_script_preserves_adgroup_campaign_name(self) -> None:
        ...
        self.assertIn('key: "ad_name", header: "ad_name"', script)
        self.assertIn("const existingHeaders = existingHeaderCount", script)
        self.assertIn("sheet.insertColumnBefore(index + 1)", script)
        self.assertIn("existingHeaders.splice(index, 0, header)", script)
        self.assertIn("브랜드명", script)
        self.assertIn("담당자명", script)
        self.assertIn("담당자 이메일", script)
        self.assertIn("settings: [", script)
        self.assertIn('{ key: "operator_name", header: "운영자" }', script)
        self.assertIn('{ key: "status_label", header: "상태" }', script)
        self.assertIn('{ key: "memo", header: "운영 메모" }', script)
        self.assertIn('payload.action === "campaign_intake_settings_update"', script)
        self.assertIn('upsertSettingRow_(buildSettingRow_', script)
        self.assertIn("function syncSettingsHeaderNow()", script)
        self.assertIn("owner_headquarters", script)
        self.assertIn('sheetName === "ops_meta"', script)
        self.assertIn('sheetName === "settings"', script)
        ...
        self.assertIn('readSheetRows_("settings")', script)
