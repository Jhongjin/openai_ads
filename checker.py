    ...
    *,
    ...
    if oai_groups:
        ...
        if _has_root_disallow(oai_groups):
            return _result(
                normalized,
                verdict="block",
                reason="OAI 봇 명시적 전체 차단",
                action="robots.txt에서 OAI-AdsBot/OAI-SearchBot 차단 규칙 수정 필요",
                http_status=http_status,
        ...
        if _has_root_disallow(star_groups):
            return _result(
                normalized,
                verdict="block",
                reason="전체 봇 차단(*) — OAI 봇도 차단됨",
                action="robots.txt의 User-agent: * 전체 차단 해제 또는 OAI 봇 예외 허용 필요",
                http_status=http_status,
                robots_txt=robots_txt,
            )
        base_reason = (
            ...
            else "전체 봇 규칙 확인 — 전체 차단 없음"
        )
    elif groups:
        return _result(
            normalized,
            verdict="warn",
            reason="특정 봇만 명시, OAI 봇 매칭 그룹 없음 — 해석상 허용될 가능성 높으나 개발팀 확인 권장",
            action="광고주 개발팀에 OAI-AdsBot/OAI-SearchBot 허용 규칙 명시 요청 권장",
            http_status=http_status,
            robots_txt=robots_txt,
        )
    else:
        base_reason = "robots.txt에 제한 규칙 없음 = 전체 허용으로 간주"
    ...
    if applied_groups:
        ...
        if matched_disallow:
            return _result(
                ...
                verdict="warn",
                reason=(
                    f"사이트는 허용이나 광고 랜딩 경로({normalized.path})가 "
                    f"Disallow에 걸림"
                ),
                action=f"Disallow: {matched_disallow} 해제 또는 다른 랜딩 경로 필요",
                http_status=http_status,
                robots_txt=robots_txt,
            )
    ...
    if not _standard_robotparser_allows(robots_txt, normalized.normalized_url):
        return _result(
            ...
            verdict="warn",
            reason=(
                f"사이트는 허용처럼 보이나 표준 파서 기준 광고 랜딩 경로"
                f"({normalized.path})가 차단됨"
            ),
            action="robots.txt의 상세 Allow/Disallow 우선순위를 개발팀에서 확인 필요",
            http_status=http_status,
            robots_txt=robots_txt,
        )
    ...
    return _result(
        ...
        verdict="allow",
        reason=base_reason,
        action="개발팀에 실제 OAI-AdsBot 접근 테스트로 최종 확인 요청",
        http_status=http_status,
        robots_txt=robots_txt,
    )
...
def _http_error_result(
    ...
    if status_code == 404:
        return _result(
            normalized,
            verdict="allow",
            reason="robots.txt 없음 = 전체 허용으로 간주 (단, 방화벽은 별도 점검)",
            action="방화벽/안티봇 차단 여부는 실제 OAI 봇 접근 테스트로 확인",
            http_status=status_code,
            robots_txt=body or "robots.txt 없음 (HTTP 404)",
        )
    ...
    if status_code == 403 and any(
        ...
        return _result(
            normalized,
            verdict="block",
            reason="Cloudflare 등 방화벽 차단 — robots 이전에 봇 차단됨",
            action="방화벽에서 OAI-AdsBot/OAI-SearchBot allowlist 필요",
            http_status=status_code,
            robots_txt=body,
        )
    ...
    return _result(
        ...
        action="서버 응답 코드와 방화벽/안티봇 정책 확인 필요",
        http_status=status_code,
        robots_txt=body,
    )
...
async def check_url(raw_url: str, client: httpx.AsyncClient) -> CheckResult:
    ...
    try:
        ...
        return _result(
            normalized,
            verdict="warn",
            reason="접근 불가(타임아웃) — 서버 응답 불안정 또는 봇차단 의심",
            action="재시도 후 지속되면 광고주 개발팀 확인 필요",
            http_status=None,
            robots_txt="",
        )
    ...
    if response.status_code == 200:
        return evaluate_robots_txt(normalized, body, http_status=response.status_code)
